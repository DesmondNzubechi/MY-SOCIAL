...
import { AllThePost } from "@/app/components/allPosts/allPost";

export const FullPost = ({params}: {params : {fullPost : string}}) => {
  const loggedInUser = userAuth();
  const [commentInput, setCommentInput] = useState<string>('');
  console.log("comment", commentInput)
 
    const allPost = AllThePost();
    const data = allPost.find((post: allPostInfo) => post.postId === params.fullPost )
        ...
        <div className=" p-2 gap-[20px] relative pb-[70px] flex-col flex"> 
            ...
            <div className="flex items-center border-t border-b bg-slate-50 py-[10px] justify-around">
              ...
              <div className=" flex items-center p-[5px] gap-x-[5px] rounded"><BiRepost className="text-[20px] " /><p className="text-slate-500">{data?.postRepost} Repost</p></div>
                ...
                {data?.postComment.length == 0 ? <h1 className="capitalize text-slate-500 text-[20px] text-center ">There is no comment under this post. be the first person to comment</h1>: 
                  data?.postComment?.map((comment: any, index: number) => {
