...
try {
  ...
  const addCommentfn = async (post: allPostInfo) => {
    ...
    {  post?.reposterName &&  <div className="bg-slate-100 p-2">
              ...
              <h1 className="font-bold flex  capitalize items-center ">  {(post?.reposterPics !== '' || post?.reposterPics) ? <Link href={`/users/${post.reposterId}`}><Image src={post?.reposterPics} height={50} width={50} className="rounded-full md:w-[50px] w-[30px] " alt="post pic" /></Link> :  <Link href={`/users/${post.reposterId}`}><FaUserCircle className="md:text-[30px] text-[15px] bg-slate-50 rounded-full shadow-2xl " /></Link>} <Link href={`/users/${post.reposterId}`}><span className="text-[10px] md:text-[20px]"> @{post.reposterName} </span></Link> </h1> <span className="text-slate-500 md:text-[20px] text-[10px] ">Reposted</span> <GoDotFill/> <p className="text-slate-500 text-[7px] md:text-[10px]">{post.respotDate}</p>
              </div>
                    <p className="text-slate-700 text-[12px] md:text-[15px] mb-[10px]">{post?.repostThought}</p>
              
                  </div>}
   <div className="flex gap-1 flex-row items-center">
       <h1 className="font-bold flex  capitalize items-center ">  {post.authorPics !== '' ? <Link href={`/users/${post?.authorId}`}><Image src={post.authorPics} height={50} width={50} className="rounded-full md:w-[50px] w-[30px]" alt="post pic" /></Link> : <Link href={`/users/${post.authorId}`}><FaUserCircle className="md:text-[30px] text-[15px] bg-slate-50 rounded-full shadow-2xl " /></Link>} <Link href={`/users/${post.authorId}`}><span className="md:text-[20px] text-[10px]">@{post.authorName}</span></Link> </h1> <span className="text-slate-500 md:text-[15px] text-[12px] ">posted</span> <GoDotFill/> <p className="text-slate-500 md:text-[12px] text-[7px]">{post.postsDate}</p>
   </div>
   <div className="">
   ...
   <div className="flex items-center border-t border-b py-[5px] justify-around">
     ...
     <div className="py-[10px] w-full bg-slate-50 flex items-center justify-around px-[20px] gap-1">
       <input onChange={(e) => setCommentInput(e.target.value)} type="text" placeholder={loggedInUser? `Comment as ${loggedInUser?.displayName}` : " Kindly Login"} className="w-full outline-none bg-transparent" />
