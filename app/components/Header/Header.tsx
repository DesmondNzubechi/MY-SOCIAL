...
export const HeaderNavLink = () => {
  ...
  useEffect(() => {
   ...
   {loggedInUser ? <ul className="flex flex-col gap-3 items-center ">
      <li className="flex flex-col   rounded-[10px] py-[10px] px-[20px] items-center">
        ...
        <Link onClick={viewProFn} className="border   w-full text-center text-slate-900 rounded p-1" href='/my-profile'>View Profile</Link>
          ...
          <li>
                <button onClick={() => {
                    viewProFn();
                    setLogOut(true)
      } } className="flex text-[20px]  items-center"><IoMdLogOut /> <span className="text-slate-500">Logout</span></button></li>
    </ul> :  <ul className="flex flex-col gap-3 items-center ">
   <li> <Link href='/' onClick={viewProFn} className="text-slate-700 text-[20px] capitalize ">Home</Link></li>
     <li> <Link href='/login' onClick={viewProFn} className="flex text-[20px] mt-[20px] items-center"><IoMdLogOut /> <span className="text-slate-500">Login</span></Link></li>
    </ul>}
  </div>} 
