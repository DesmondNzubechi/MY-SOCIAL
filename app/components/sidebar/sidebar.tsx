...
export const SideBar = ({ setPublishPost }: { setPublishPost: React.Dispatch<React.SetStateAction<string>> }) => {
  const loggedInUser = userAuth();
   ...
   {loggedInUser ? <ul className="flex flex-col gap-3 items-center ">
     ...
     {loggedInUser?.photoURL?  <Image className="rounded-full" alt={`${loggedInUser?.displayName} profile pic`} src={loggedInUser?.photoURL} height={50} width={50} /> : <FaUserCircle className="text-[70px] bg-slate-50 rounded-full  " />}
